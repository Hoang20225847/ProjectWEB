...
function ManageSeries() {
  ...
  const [membersSeries, setMembersSeries] = useState(null);
  ...
  const load = useCallback(async () => {
    try {
      ...
      toast.error('Không tải được danh sách series');
      setRows([]);
    }
  }, []);
  ...
  const handleCreate = async (e) => {
    ...
    if (!name) {
      toast.error('Nhập tên series');
      return;
    }
    try {
      await axios.post('/api/series', { name, description: createDesc.trim() });
      ...
      toast.error(err?.response?.data?.message || 'Không tạo được series');
    }
  };
  ...
  const handleSaveEdit = async (e) => {
    ...
    if (!editRow) return;
    try {
      await axios.put(`/api/series/${editRow._id}`, {
  ...
  const openMembers = async (row) => {
    setMembersSeries(row);
    setBookSearch('');
    try {
      const [inSeries, all] = await Promise.all([
        axios.get(`/api/series/${row._id}/books`),
        getBookList(),
      ]);
      const inArr = Array.isArray(inSeries) ? inSeries : [];
      const books = Array.isArray(all) ? all : [];
      setAllBooks(books);
      setMemberIds(new Set(inArr.map((b) => String(b._id))));
    } catch {
      toast.error('Không tải danh sách sách');
      setMembersSeries(null);
    }
  };
  ...
  const saveMembers = async () => {
    if (!membersSeries) return;
    setSavingMembers(true);
    try {
      await axios.put(`/api/series/${membersSeries._id}/members`, {
        bookIds: [...memberIds],
      });
      toast.success('Đã cập nhật sách trong series');
      setMembersSeries(null);
      await load();
    } catch (err) {
  ...
  return (
    <div className={cx('resourceCatalog')}>
      <div className={cx('admin-nav')}>
        <div className={`${cx('logo-search')} admin-title`}>
          <div>
            ...
            <span className="admin-title-name">Quản lý bộ sách (Series)</span>
          </div>
        </div>
        <div className={cx('resourceCatalogToolbar')}>
          ...
          <button type="button" className={cx('btn-controll')} onClick={() => setCreateOpen((o) => !o)}>
            <i className="fa-solid fa-plus" /> Series mới
          </button>
        </div>
      </div>
      ...
      <div className={cx('resourceCatalogTable')}>
        ...
        {filtered.length === 0 && (
          <p className={cx('resourceCatalogEmpty')}>Chưa có series nào. Tạo series mới để gom bộ sách.</p>
        )}
      </div>
      ...
      {membersSeries && (
        <div className={cx('modalOverlay')} onClick={() => !savingMembers && setMembersSeries(null)}>
          ...
          >
            <div className={cx('modalHeader')}>
              <h3>Chọn sách — {membersSeries.name}</h3>
              <button type="button" className={cx('modalClose')} disabled={savingMembers} onClick={() => setMembersSeries(null)}>
                <i className="fa-solid fa-xmark" />
              </button>
            </div>
            ...
            <div className={cx('modalFooter')}>
              <button type="button" className="btn btn--secondary" disabled={savingMembers} onClick={() => setMembersSeries(null)}>
                Đóng
              </button>
